namespace non_blocking
{
	...
	namespace json
	{
		...
		//entry point
		void writeFiles(const ClassObject& m_ob, std::ostream& hos, std::ostream& cos, std::vector<std::string>& namespaces/*, const std::string& header*/);

		//utils
		void writeStructureItems(const ClassObject* pObj, std::stringstream& cos, std::string& dereferencer);

		//parse generation
		bool readTypeAtomic(std::ostream& cos, std::string& paramType, int next_pos, const char* field, int comma_count);
		...
		bool writeType(std::ostream& cos, std::string& paramType, const char* paramName, const Library& library);
	}
}