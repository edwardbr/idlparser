...
ostream& operator<< ( ostream& os, attributes& attribs )
{
	if(attribs.size())
	{
		os << '[';
		bool firstpass = true;
		for(list<string>::iterator it = attribs.begin();it != attribs.end();it++)
		{
			if(!firstpass)
				os << ',' ;
			else
...
ostream& operator<< ( ostream& os, ParameterObject& parameter)
{
...
ostream& operator<< ( ostream& os, FunctionObject& function)
{
	os << '\t';
	os << function.m_attributes;
	os << function.returnType << " " << function.name << "(";

	for(list<ParameterObject>::iterator it = function.parameters.begin();it != function.parameters.end();it++)
	{
...
void ClassObject::AddClass(RefCountSmartPtr<ClassObject> classObject)
{
...
ostream& operator<< ( ostream& os, ClassObject& other)
{
	...
	case ObjectStruct:
		{
			...
			if(other.parentName.length())
				os << " : " << other.parentName;
			os << endl << '{' << endl;
			for(list<FunctionObject>::iterator it = other.functions.begin();it != other.functions.end();it++)
				os << *it;
			os << "};" << endl << endl;
			break;
		}
	...
	default:
		verboseStream << "arrg unknown type" << endl;
	}
#ifdef _DEBUG
	verboseStream.flush();
#endif
...
bool Library::FindClassObject(const string& type, const ClassObject*& obj) const
{
	string typ = type;
	string namesp;
