                                            ...
                                            std::vector<std::string>& loaded_includes)
{
    ...
    while (*pData != '\0' && *pData != '\n')
    {
        ...
        else if (*pData == '!' || *pData == '"' || *pData == '%' || *pData == '^' || *pData == '&' || *pData == '*'
                 ...
                 || *pData == '\n')
        {
            ...
            while (*pData == '!' || *pData == '"' || *pData == '%' || *pData == '^' || *pData == '&' || *pData == '*'
                   || *pData == '+' || *pData == '=' || *pData == '{' || *pData == '}' || *pData == '~' || *pData == '/'
...
void macro_parser::ProcessIf(const char*& pData, std::ostream& dest, const paths& includeDirectories, int& inIfDef,
...
void macro_parser::CleanBuffer(const char*& pData, std::ostream& dest, const paths& includeDirectories, int inIfDef,
                               int ignoreText, std::vector<std::string>& loaded_includes)
{
    ...
    while (*pData != 0)
    {
        ...
        else
        {
            if (*pData != '\n')
            {
                if (*pData == ' ' || *pData == '!' || *pData == '\"' || *pData == '~' || *pData == '^' || *pData == '&'
                    || *pData == '*' || *pData == '(' || *pData == ')' || *pData == '}' || *pData == '{'
                    || *pData == '[' || *pData == ']' || *pData == ';' || *pData == ':' || *pData == '\''
                    || *pData == '<' || *pData == '>' || *pData == '.' || *pData == ',' || *pData == '/'
                    || *pData == '\\' || *pData == '|')
                                           ...
                                           std::vector<std::string>& loaded_includes)
{
    ...
    // Having reduced the expresion to a list of values sandwiched between operatorBehaviours we now evaluate the
    // expression ( ) Function call   //should now be evaluated
...
bool macro_parser::SubstituteMacro(int ignoreText, const char*& pData, std::ostream& dest,
                                   const paths& includeDirectories, std::vector<std::string>& loaded_includes)
{
    ...
    {
        // copy spaces;
        while ((def.m_substitutionString[i] == ' ' || def.m_substitutionString[i] == '('
                || def.m_substitutionString[i] == ')')
               && def.m_substitutionString.length() > i)
            newSubstString += def.m_substitutionString[i++];

        // now extract a morcel of std::string data
        std::string morcel;
        while (def.m_substitutionString[i] != ' ' && def.m_substitutionString[i] != '#'
               && def.m_substitutionString[i] != '(' && def.m_substitutionString[i] != ')'
               && def.m_substitutionString.length() > i)
...
bool macro_parser::load(std::ostream& output_file, const std::string& file, const paths& includeDirectories,
                        std::vector<std::string>& loaded_includes)
{
...
void macro_parser::extract_path_and_load(int ignoreText, std::ostream& stream, const paths& includeDirectories,
...
void CleanBufferOfComments(const char*& pData)
{
    ...
    while (changed)
    {
        ...
        while (*pData != 0)
        {
            ...
            // #defines and conditional statements need special handling and are characterised by being terminated by a
            // charage return
            else if (is_preproc(pData, "define") || is_preproc(pData, "ifdef") || is_preproc(pData, "ifndef")
                     || is_preproc(pData, "if") || is_preproc(pData, "elif") || is_preproc(pData, "endif")
                     || is_preproc(pData, "error"))
            {
                if (oldBufPos != pData)
                {
                    *oldBufPos++ = '\n';
