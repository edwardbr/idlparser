...
namespace javascript_json
{
	namespace json
	{
		...
		void writeFunction(const std::string& paramType, const Library& library, ostringstream* outerjos = NULL);
		...
		void writeJSONFiles(const ClassObject& m_ob, ostream& jos)
		{
			...
			ostringstream tmpjos;//throw away
			ostringstream fnjos;//throw away
		...
		void writeFunctionParameters(const std::string& type, const Library& library, ostringstream& jos, bool& includeComma)
		{
			if(p_timer != NULL && p_timer->is_timedOut())
			{
				exit(0);
		...
		void writeFunction(const std::string& paramType, const Library& library, ostringstream* outerjos)
		{
			if(p_timer != NULL && p_timer->is_timedOut())
			{
				exit(0);
