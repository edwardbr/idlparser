			...
			while(*pData != 0)
			{
				...
				if(*pData == '(')
				{
					...
					if(*pData == ')')
					{
						...
						cerr << "not a call back as expected";
				}
				else
				{
...
//#############this code was commented out doing so will break the com/corba bridge							
					for(list<ClassObject*, ClassObjectAllocator>::iterator it = pObjects->m_classes->begin();it != pObjects->m_classes->end();it++)
					{
	...
	while(!bFunctionIsProperty && *pData != 0 && *pData != ';')
	{
		...
		if(*pData == '{')
		{
			...
			EAT_SPACES(pData)
			if(*pData != '}')
				cerr << "function implementations not supported";
			else
				pData++;
			break;
		}
...
ClassObject::ClassObjectRef ClassObject::GetInterface(const char*& pData, const ObjectType typ, const attributes& attr, const std::string& ns)
{
	...
	cls.val().type = typ;
	cls.val().m_attributes = attr;

	cls.val().GetStructure(pData, ns);
	return cls;
}
...
void ClassObject::GetStructure(const char*& pData, const std::string& ns, bool bInCurlyBrackets)
{
	...
	//					assert(*pData == ';');
					...
					else if(type == ObjectUnion)
					{
						...
						{
							attribs.push_back(string("default"));
		...
		else
		{
			...
			if(type == ObjectUnion)
			{
				if(IfIsWordEat(pData,"switch"))
				{
					...
					attributes attribs(GetAttributes(pData));
					attribs.push_back(string("switch"));
...
ClassObject::ClassObjectRef ClassObject::ParseSequence(const char*& pData, attributes& attribs, const std::string& ns)
{
	...
	{
		newInterface.val().parentName += *pData;
		pData++;
	}
	...
	while(*pData != ';' && *pData != 0)
		newInterface.val().name += *pData++;

	verboseStream << "sequence<" << newInterface.val().parentName.data() << "> " << newInterface.val().name.data() << ";" << endl;
	
	newInterface.val().type = ObjectSequence;
	newInterface.val().m_attributes = attribs;
...
ClassObject::ClassObjectRef ClassObject::ParseTypedef(const char*& pData, attributes& attribs, const std::string& ns, const char* type)
{
	...
	object.val().type = ObjectTypedef;

	attribs.merge(GetAttributes(pData));

	ClassObjectRef obj(new ClassObject(object.ptr(),&object.val().GetLibrary(), ns));
	if(type == NULL && object.val().ExtractClass(pData, attribs,obj, ns,false))
	{
		object.val().parentName = obj.val().name;
		bool firstPass = true;
		do
		{
			if(p_timer != NULL && p_timer->is_timedOut())
			...
			if(ispointer)
				temp.val().m_attributes.push_back(string("pointer"));
			if(firstPass)
			{
				firstPass = false;
				object.val().name = temp.val().name;
			}
...
//		assert(*pData == ';');
	...
	else
	{
		...
		while(*pData != 0 && *pData != ';')
		{
			...
			if(object.val().parentName.length())
				object.val().parentName += ' ';

			object.val().parentName += object.val().name;
			object.val().name = "";
			...
			if(*pData == ',')
			{
				pData++;
				EAT_SPACES(pData)
				string& name = object.val().parentName;
				string type;
...
//		pData++;
	...
	verboseStream << "typedef " << object.val().name.data() << " " << object.val().name.data() << endl;
...
{
	verboseStream << "skipping past a union" << endl;
	while(*pData && *pData != ';')
	{
		if(p_timer != NULL && p_timer->is_timedOut())
		{
...
bool ClassObject::ExtractClass(const char*& pData, attributes& attribs, ClassObjectRef& obj, const std::string& ns, bool handleTypeDefs)
{
	...
	else if(IfIsWordEat(pData,"library"))
	{
		if(isHashImport)
		{
			cerr << "encountered a library inside a #import" << endl;
			Library lib;
	...
	{
		if(isHashImport)
			cerr << "encountered a coclass inside a #import" << endl;
...
/*bool ClassObject::LoadUsingEnv(const string& file)
{
...
string ClassObject::GenerateTypeString(TYPEDESC& typedesc, ITypeInfo* typeInfo)
{
	USES_CONVERSION;
	if (typedesc.vt == VT_USERDEFINED)
	{
		...
		if (SUCCEEDED(hr))
			return string(W2CA(GetInterfaceName(userTypeInfo)));
	}
	
	if (typedesc.vt == VT_CARRAY)
	{
		string ret(GenerateTypeString(typedesc.lpadesc->tdescElem, typeInfo));
	...
	if (typedesc.vt == VT_PTR || typedesc.vt == VT_SAFEARRAY)
		return string(GenerateTypeString(*typedesc.lptdesc, typeInfo) + "*");
...
/*void ClassObject::ParseBuffer(const char* pData, const char* file, const std::string& ns)
{
	std::string nameSpace;
	GetStructure(pData, nameSpace, true);
}*/
...
bool ClassObject::Load(const char* file)
{
	...
	std::error_code ec;
	std::string path(std::experimental::filesystem::canonical(file, ec).string());


	ifstream preproc_stream(path);
	if(preproc_stream.is_open() == false)
	{
		return 0;
	}
	string preproc_data = std::string(std::istreambuf_iterator<char>(preproc_stream), std::istreambuf_iterator<char>());
